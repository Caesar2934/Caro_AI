    ...
    for (let r = 0; r < BOARD_SIZE; ++r) {
        for (let c = 0; c < BOARD_SIZE; ++c) {
            ...
            for (let d = 0; d < 4; ++d) {
                ...
                total_score += evaluate_chain(count, open_ends, is_ai);
            }
        }
    }
    return total_score;
...
function get_candidates(board) {
    ...
    for (let cell of empty_cells) {
        ...
        if (found_near) {
            let score = -(Math.abs(r - 7) + Math.abs(c - 7));
...
function minimax(board, depth, alpha, beta, is_maximizing, ai_player) {
    let base_eval = evaluate_board_heuristic(board, ai_player); if(base_eval >= 9000000) console.log('WIN SCORE:', base_eval);
    if (base_eval >= 9000000) return {score: base_eval + depth, r: -1, c: -1};
    if (base_eval <= -9000000) return {score: base_eval - depth, r: -1, c: -1};

    if (depth === 0) return {score: base_eval, r: -1, c: -1};

    let candidates = get_candidates(board);
...
function get_best_move(board, player_turn, max_depth) {
    ...
    if (result.r === -1) {
        // Fallback
        let cands = get_candidates(board);
